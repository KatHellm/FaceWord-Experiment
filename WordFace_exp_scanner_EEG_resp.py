    ...
    for trial in make_trial_list(condition, run):
        ...
        for frame in range(trial['duration_frames']):
            ...
            if frame>1 and no_key_yet == 0:
                try:
                    key, time_key = event.getKeys(keyList=('e', 'i', '1', '2', 'escape'), timeStamped=True)[0]  # timestamped according to core.monotonicClock.getTime() at keypress
...
for run in range(RUNS):
    ...
    introText1=[u'In this experiment you will read words and look at faces', # some blanks here to create line shifts
                ...
                u'Press "i" key with INDEX finger if face is POSITIVE',

                u'Press "e" key with MIDDLE finger if face is NEGATIVE',
